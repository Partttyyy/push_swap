...
/*   Updated: 2023/11/06 19:49:51 by frapp            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_charset(char *arg, t_both *both)
{
	...
	while (arg[++i])
	{
		...
		if (!ft_isdigit(arg[i]))
		{
			if (was_sign || (arg[i] != ' ' && arg[i] != '-' && arg[i] != '+'))
				return (0);
			if (nb_c)
			{
				if (!(append_back(both->sa, ft_atoi(arg), both)))
					return (0);
				count++;
			}
			nb_c = 0;
	...
	if (nb_c)
	{
		both->sa = append_back(both->sa,ft_atoi(arg), both);
		if (!(both->sa))
			return (0);
		count++;
	}
